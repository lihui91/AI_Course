...
int search_depth = 4;//搜索深度

pair<int, int> searchMove(int player) //搜索函数主体
{
	int depth = steps.size() / 40;//?
	search_depth = 4 + depth;
	alphaBeta(player, INT_MIN, INT_MAX, search_depth, player, 0);//初始状态alpha为负无穷 ，beta为正无穷，分值为0
	return make_pair(to_move_x, to_move_y);//返回最佳落子位置
}
